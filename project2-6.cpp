...
cout << s.row << " " << s.col << "  ";
    ...
    if(s.row == 0){ //if neigbors are all visited
        if(s.col == 0){
            if(visit[s.row+1][s.col] != 0 && visit[s.row][s.col+1] != 0) return;
        }else if(s.col == col_bound-1){
            if(visit[s.row][s.col-1] != 0 && visit[s.row][s.col+1] != 0) return;
        }else{
            if(visit[s.row+1][s.col] != 0 && visit[s.row][s.col+1] != 0 && visit[s.row][s.col-1] != 0) return;
        }
    }
    else if(s.row == row_bound-1){
        if(s.col == 0){
            if(visit[s.row-1][s.col] != 0 && visit[s.row][s.col+1] != 0) return;
        }else if(s.col == col_bound-1){
            if(visit[s.row-1][s.col] != 0 && visit[s.row][s.col-1] != 0) return;
        ...
        if(s.col == 0){
            if(visit[s.row-1][s.col] != 0 && visit[s.row+1][s.col] != 0 && visit[s.row][s.col+1] != 0) return;
        }else if(s.col == col_bound-1){
            if(visit[s.row-1][s.col] != 0 && visit[s.row+1][s.col] != 0 && visit[s.row][s.col-1] != 0) return;
        }else{
            if(visit[s.row-1][s.col] != 0 && visit[s.row+1][s.col] != 0 
               && visit[s.row][s.col-1] != 0 && visit[s.row][s.col+1] != 0) return;
        }
    }//
//cout << "OK ";
    ...
    for(int i=0; adj[s.row][s.col][i].row != INT_MIN+5; i++){
